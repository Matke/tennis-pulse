...
import { FaPeopleArrows, FaListAlt } from "react-icons/fa";
import { IoTennisball } from "react-icons/io5";
import { RiGroup3Fill, RiSidebarFoldFill } from "react-icons/ri";
...
const navigation: NavigationItem[] = [
  ...
  },
  {
    ...
    subMenu: [
      { key: "new", icon: <FaHandshake className="text-xl" /> },
      { key: "matchups", icon: <RiGroup3Fill className="text-xl" /> },
...
const Sidebar = () => {
  ...
  return (
    ...
    <aside
      className={`${open ? "w-70 p-3" : "w-20 p-3"} bg-tp-main-background relative flex h-screen flex-col pt-5`}
    >
      ...
      <div className="scrollbar-hide flex-1 space-y-0.5 overflow-y-auto pt-2">
        {navigation.map((item: NavigationItem) => {
          // to have access whether item (main route) is active or not in Link below
          const isParentActive = location.pathname.startsWith(`/${item.key}`);
                  ...
                  {item.subMenu.map((subMenu: SubMenuItem) => (
                    ...
                    >
                      ...
                      {subMenu.key.charAt(0).toUpperCase() +
                        subMenu.key.split("-").join(" ").slice(1)}
                    </Link>
                  ))}
                </ul>
              )}
      ...
      >
        ...
        <Dropdown
          buttonIcon={
            <ButtonIcon
              icon={<HiDotsVertical />}
              className={`${!open && "hidden"} hover:bg-zinc-800`}
              themeColor="blank"
